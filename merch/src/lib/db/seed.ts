...
async function seed() {
  ...
  log('Seeding brand: Better Auth Merch Website');
  const brand = insertBrandSchema.parse({ name: 'Better Auth Merch Website', slug: 'better-auth-merch', logoUrl: undefined });
    {
  ...
  const merchBrand = (await db.select().from(brands).where(eq(brands.slug, 'better-auth-merch')))[0];
  ...
  const productNames = Array.from({ length: 15 }, (_, i) => `Better Auth Merch Tee ${i + 1}`);
  ...
  brandId: merchBrand?.id ?? null,
      ...
      for (const color of colorChoices) {
        for (const size of sizeChoices) {
          ...
          const sku = `MERCH-${insertedProduct.id.slice(0, 8)}-${color.slug.toUpperCase()}-${size.slug.toUpperCase()}`;
